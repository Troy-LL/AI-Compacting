"""Tests for the LifeLink RWKV model.
...
class TestLifeLinkForwardPass:

    def test_forward_shape(self, tiny_lifelink_model: LifeLinkRWKV) -> None:
        """Forward pass produces (B, T, vocab_size) logits."""
        cfg = tiny_lifelink_model.config
        x = torch.randint(0, cfg.vocab_size, (2, 16))
        with torch.no_grad():
            logits, h_states = tiny_lifelink_model(x)
        assert logits.shape == (2, 16, cfg.vocab_size), (
            f"Expected (2, 16, {cfg.vocab_size}), got {logits.shape}"
        )

    def test_h_states_returned(self, tiny_lifelink_model: LifeLinkRWKV) -> None:
        """Forward pass returns one h_state per layer."""
        cfg = tiny_lifelink_model.config
        x = torch.randint(0, cfg.vocab_size, (2, 8))
        with torch.no_grad():
            _, h_states = tiny_lifelink_model(x)
        assert len(h_states) == cfg.layers, (
            f"Expected {cfg.layers} states, got {len(h_states)}"
        )

    def test_h_state_shapes(self, tiny_lifelink_model: LifeLinkRWKV) -> None:
    ...
    def test_logits_are_finite(self, tiny_lifelink_model: LifeLinkRWKV) -> None:
        """No NaN or Inf in logits at initialisation."""
        cfg = tiny_lifelink_model.config
        x = torch.randint(0, cfg.vocab_size, (2, 16))
        with torch.no_grad():
            logits, _ = tiny_lifelink_model(x)
        assert torch.isfinite(logits).all(), "Logits contain NaN or Inf"

    def test_stateful_inference(self, tiny_lifelink_model: LifeLinkRWKV) -> None:
        ...
        with torch.no_grad():
            _, h1 = tiny_lifelink_model(x1)
            logits2, h2 = tiny_lifelink_model(x2, h_states=h1)
...
class TestLifeLinkParameterCount:

    def test_parameter_count_in_expected_range(self, lifelink_model: LifeLinkRWKV) -> None:
        """LifeLink unique parameter count must be in the expected range.
        ...
        params = lifelink_model.num_parameters()
    ...
    def test_parameter_count_nonzero(self, lifelink_model: LifeLinkRWKV) -> None:
        """Model must have parameters."""
        assert lifelink_model.num_parameters() > 0


# ── Parameter sharing ─────────────────────────────────────────────────────────

class TestLifeLinkParameterSharing:

    def test_parameter_sharing_active(self, tiny_lifelink_model: LifeLinkRWKV) -> None:
        ...
        ffn_layer0 = tiny_lifelink_model.blocks[0].ffn
        ffn_layer_gs = tiny_lifelink_model.blocks[group_size].ffn
    ...
    def test_shared_ffn_param_data_ptr(self, tiny_lifelink_model: LifeLinkRWKV) -> None:
        """Shared FFN weights have identical data pointers (same storage)."""
        cfg = tiny_lifelink_model.config
        group_size = cfg.ffn_sharing_group_size

        w0 = tiny_lifelink_model.blocks[0].ffn.fc1.weight
        w_gs = tiny_lifelink_model.blocks[group_size].ffn.fc1.weight

        assert w0.data_ptr() == w_gs.data_ptr(), (
            "Shared FFN weights should point to the same memory"
        )

    def test_total_unique_ffn_blocks(self, tiny_lifelink_model: LifeLinkRWKV) -> None:
        """Number of unique FFN nn.Module instances = group_size (not num_layers)."""
        cfg = tiny_lifelink_model.config
        ffn_set = {id(block.ffn) for block in tiny_lifelink_model.blocks}
...
class TestLifeLinkFixedState:
    """Proves that LifeLink RWKV's memory footprint is constant regardless of seq len."""

    def test_state_is_fixed_size(self, tiny_lifelink_model: LifeLinkRWKV) -> None:
        ...
        with torch.no_grad():
            _, h_short = tiny_lifelink_model(x_short)
            _, h_long = tiny_lifelink_model(x_long)
    ...
    def test_state_byte_count_independent_of_sequence(
        self,
        tiny_lifelink_model: LifeLinkRWKV,
    ) -> None:
...
class TestLifeLinkLowRankProjections:
    ...
    def test_attention_projections_are_low_rank(
        ...
        for i, block in enumerate(tiny_lifelink_model.blocks):
    ...
    def test_low_rank_dim_respected(self, tiny_lifelink_model: LifeLinkRWKV) -> None:
