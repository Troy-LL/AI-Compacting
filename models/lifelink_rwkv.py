"""LifeLink RWKV — recurrent architecture with fixed memory footprint.
...
@dataclass
class LifeLinkConfig:
    """Configuration for the LifeLink RWKV model."""
    ...
    @classmethod
    def from_yaml(cls, path: str | Path) -> "LifeLinkConfig":
...
class LifeLinkBlock(nn.Module):
    """LifeLink transformer block.
...
class LifeLinkRWKV(nn.Module):
    """LifeLink RWKV — recurrent model with fixed memory footprint.

    The innovation model for H(AI)LP. Replaces the growing KV cache with
    a fixed-size recurrent state.

    Parameters
    ----------
    config : LifeLinkConfig
        Model configuration (loaded from configs/lifelink.yaml).

    Example
    -------
    >>> cfg = LifeLinkConfig()
    >>> model = LifeLinkRWKV(cfg)
    ...
    def __init__(self, config: LifeLinkConfig) -> None:
        super().__init__()
        self.config = config
        ...
        self.blocks = nn.ModuleList([
            LifeLinkBlock(config, self.ffn_pool, layer_idx=i)
    ...
    @classmethod
    def from_config_file(cls, path: str | Path) -> "LifeLinkRWKV":
        """Construct a LifeLinkRWKV from a YAML config file."""
        return cls(LifeLinkConfig.from_yaml(path))
